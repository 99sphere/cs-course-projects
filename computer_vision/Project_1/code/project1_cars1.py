...
if __name__=="__main__":
    ...
    padded_img = np.lib.pad(img, (((average_filter_size-1)//2, (average_filter_size-1)//2),((average_filter_size-1)//2, (average_filter_size-1)//2)), 'reflect')
    ...
    print("Done average filtering in spatial domain")
    ...
    saveimage(grad_x_img, "output/cars1/3_sobel_x.png")

    grad_y = maskfiltering(padded_img, sobel_y)
    max = grad_y.max()
    min = grad_y.min()
    grad_y_img = (grad_y - min) / (max-min) * 255  
    saveimage(grad_y_img, "output/cars1/4_sobel_y.png")

    grad_img = np.sqrt((grad_x)**2 + grad_y**2)
    max = grad_img.max()
    min = grad_img.min()
    grad_img = (grad_img - min) / (max-min) * 255  
    saveimage(grad_img, "output/cars1/5_sobel_final.png")
    
    # applying padding and butterworth highpass filter in frequency domain
    half_row = row // 2
    half_col = col // 2
    ...
    saveimage(HPF_mask*255, f"output/cars1/temp_BHPF_mask.png")
        
    img_fft_filtered = np.multiply(img_fft, HPF_mask)

    img = imageifft(img_fft_filtered, center=True)
    img = np.abs(img[half_row:-half_row, half_col:-half_col])

    img -= np.min(img)
    img /= np.max(img)
    img *= 255
    saveimage(img, f"output/cars1/temp_highlighted_edge.png")